...
void Renderer3D::InitGBuffer(int width, int height)
{
    ...
    lighting_pass_shader = new Shader(ShaderLoaderSystem::CreateShader("res/shaders/lighting_pass.glsl"));

    stencil_pass_shader = new Shader(ShaderLoaderSystem::CreateShader("res/shaders/stencil.glsl"));

    composite_pass_shader = new Shader(ShaderLoaderSystem::CreateShader("res/shaders/composite_pass.glsl"));
}
...
void Renderer3D::LightPass(RenderContext context)
{
    ...
    float radius = (attenuation.y + sqrtf(pow(attenuation.y, 2) - (4 * attenuation.z * (attenuation.x - 256)))) /
                   (2 * attenuation.z);

    lighting_pass_shader->SetUniformMatrix4fv("projection", glm::value_ptr(context.camera_component.GetProjectionMatrix(
                                                                context.dimensions.x, context.dimensions.y)));
    lighting_pass_shader->SetUniformMatrix4fv("view", glm::value_ptr(view));
    lighting_pass_shader->SetUniform3f("worldPos", lightPos);
    lighting_pass_shader->SetUniform1f("radius", 2);
    lighting_pass_shader->SetUniform3f("viewPos", context.camera_transform.position);
    lighting_pass_shader->SetUniform3f("lightPosition", glm::vec3(view * glm::vec4(lightPos, 1)));
    lighting_pass_shader->SetUniform3f("lightColor", glm::vec3(1, 1, 1));
    lighting_pass_shader->SetUniform3f("lightAttenuation", attenuation);
    lighting_pass_shader->SetUniform2f("screenSize", context.dimensions);

    // Set the textures
    lighting_pass_shader->SetUniform1i("positionMap", 0);
    lighting_pass_shader->SetUniform1i("normalMap", 1);
    lighting_pass_shader->SetUniform1i("colorSpecularMap", 2);
...
void Renderer3D::StencilPass(RenderContext context)
{
    ...
    stencil_pass_shader->SetUniformMatrix4fv("view", glm::value_ptr(context.camera_transform.GetViewMatrix()));
    stencil_pass_shader->SetUniformMatrix4fv("projection", glm::value_ptr(context.camera_component.GetProjectionMatrix(
                                                               context.dimensions.x, context.dimensions.y)));
...
void Renderer3D::CompositePass(RenderContext context)
{
    ...
    composite_pass_shader->Bind();
    composite_pass_shader->SetUniform1i("positionMap", 0);
    composite_pass_shader->SetUniform1i("normalMap", 1);
    composite_pass_shader->SetUniform1i("colorSpecularMap", 2);
    composite_pass_shader->SetUniform1i("lightMap", 3);
    composite_pass_shader->SetUniform3f("l",
                                        glm::vec3(context.camera_transform.GetViewMatrix() * glm::vec4(1, 1, 1, 0)));
...
void Renderer3D::ForwardRenderingPass(RenderContext context)
{
    g_buffer->BindRead();
    g_buffer->UnbindWrite();
    glBlitFramebuffer(0, 0, context.dimensions.x, context.dimensions.y, 0, 0, context.dimensions.x,
                      context.dimensions.y, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
    g_buffer->Unbind();
    glDepthMask(GL_TRUE);
    glEnable(GL_DEPTH_TEST);
...
bool Renderer3D::Register(Entity entity)
{
    ...
    std::unordered_map<unsigned int, RenderGroup *> &groups =
        shader_component.type == Forward ? render_groups_forward : render_groups_deferred;
    ...
    {
        groups[shader_component.GetShader()->GetID()] = new RenderGroup(shader_component.GetShader());
    }
