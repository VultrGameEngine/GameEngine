...
std::shared_ptr<MeshLoaderSystem> MeshLoaderSystem::RegisterSystem() {
  std::shared_ptr<MeshLoaderSystem> ptr =
      World::RegisterSystem<MeshLoaderSystem>();
  ptr->signature.set(World::GetComponentType<StaticMeshComponent>());
  World::SetSignature<MeshLoaderSystem>(ptr->signature);
...
void MeshLoaderSystem::OnCreateEntity(Entity entity) {
  ...
  if (!isLoaded(component.path)) {
    ...
    std::thread loadingThread(Import, component.path,
                              std::ref(state.meshes[component.path]));
...
void MeshLoaderSystem::Import(std::string filepath, LoadedStaticMesh &mesh) {
  ...
  MeshLoaderSystem::IndexVBO(temp_out_vertices, temp_out_uvs, temp_out_normals,
