...
void ControllerSystem::Init(GLFWwindow *window) { this->window = window; }
...
void ControllerSystem::WindowFocusCallback(GLFWwindow *window, int focused) {
  ...
  if (instance->focused) {
    glfwSetCursorPos(window, RenderSystem::GetDimensions(GAME).x / 2,
...
std::shared_ptr<ControllerSystem> ControllerSystem::RegisterSystem() {
  ...
  World::SetSignature<ControllerSystem>(ptr->signature);
  return ptr;
}
